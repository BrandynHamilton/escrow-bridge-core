"""
Test script for the async Escrow Bridge SDK.
...
async def test_async_sdk():
    ...
    async with AsyncEscrowBridgeSDK(BASE_URL, CLIENT_API_KEY) as sdk:
        # Test health
        print("\n[1] Testing health()...")
        try:
        ...
        if TEST_ESCROW_ID:
            ...
            # Uncomment to test:
            try:
                result = await sdk.webhook(TEST_WEBHOOK_URL, TEST_ESCROW_ID)
                print(f"    Result: {result}")
                print("    PASSED")
            except Exception as e:
                print(f"    FAILED: {e}")
...
def main():
    print(f"Testing async SDK against: {BASE_URL}")
